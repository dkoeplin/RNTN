	...
	def main() = {
		...
		val ssWv = DenseMatrix[Double](Wv.numRows, Wv.numCols)
	   	...
	   	while (runIter <= runsThroughData) {
			...
			println("Checking accuracy for Train Set on run " + runIter + "/" + runsThroughData + "...")
			evalOnTrees(trainTrees, Wc, W, Wt, Wv, 192)
			println("Completed accuracy check for Train Set on run " + runIter + "/" + runsThroughData)

			println("Checking accuracy for Dev Set on run " + runIter + "/" + runsThroughData + "...")
			evalOnTrees(devTrees, Wc, W, Wt, Wv, 192)
			println("Completed accuracy check for Dev Set on run " + runIter + "/" + runsThroughData)
			runIter += 1
		}		
