...
trait RNTNTrainer extends OptiMLApplication with RNTNOps with Utilities {
	...
	def main() = {
		...
		val numDevEvalBatches   = ceil(devTrees.length.toDouble/EVALBATCHSIZE)
		val numTestEvalBatches  = ceil(testTrees.length.toDouble/EVALBATCHSIZE)

		println("Training " + trainTrees.length + " training set trees in " + numTrainBatches + " batches")
		println("Evaluating " + trainTrees.length + " training set trees in " + numTrainEvalBatches + " batches")
		println("Evaluating " + devTrees.length + " dev set trees in " + numDevEvalBatches + " batches")
		println("Evaluating " + testTrees.length + " test set trees in " + numTestEvalBatches + " batches")		
	   	...
	   	while (runIter <= RUNSTHROUGHDATA) {
			...
			evalOnTrees(devTrees, Wc, W, Wt, Wv, EVALBATCHSIZE, numDevEvalBatches)
			println("-----------------------------------------------------------------------------")
			println("")
		...
		evalOnTrees(testTrees, Wc, W, Wt, Wv, EVALBATCHSIZE, numTestEvalBatches)
		println("-----------------------------------------------------------------------------")
	}	// end of main
